...
export default {
  ...
  // 注意: 这个钩子, 不会在动态路由切换的时候执行, 只会在第一次键入的时候执行
  beforeRouteEnter (to, from, next) {
    console.group('你要进入 page 组件了')
    ...
    // 判断你的 current 传递的不是一个数字的时候
    // 比如: /list/page/aaa 的时候, 不应该正常显示页面
    // 跳转到 /list/page/1
    if (isNaN(current)) {
      console.log('你要去的是 aaa 页面, 目前未开放!, 即将跳转到 1 页面')
      return this.$router.replace('/list/page/1')
    }
    // 放行
    next()
  }
